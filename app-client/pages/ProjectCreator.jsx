...
export default function ProjectCreator() {
  ...
  // Charger les templates disponibles au démarrage
  useEffect(() => {
  ...
  const loadAvailableTemplates = async () => {
    try {
      const response = await fetch(
        "http://localhost:3000/projects/meta/templates"
      );
      ...
      if (data.success) {
        ...
        // Fallback si erreur
        setAvailableTemplates([{ id: "basic", name: "Basic" }]);
      }
    } catch (error) {
      // Fallback si erreur
      setAvailableTemplates([{ id: "basic", name: "Basic" }]);
  ...
  const validateForm = () => {
    ...
    if (!formData.projectId.trim()) {
      ...
      newErrors.projectId =
        "ID doit contenir seulement des lettres minuscules, chiffres et tirets";
    ...
    // Validation Template
    if (!availableTemplates.find((t) => t.id === formData.template)) {
