...
export async function manageContainers(projectPath, action, config = {}) {
  ...
  ${projectId}-visitor:
    ...
    restart: unless-stopped`;
      ...
      execSync(`cd ${containersPath} && docker-compose create`, {
        encoding: "utf8",
      });
    }

    if (action === "start") {
      execSync(`cd ${containersPath} && docker-compose up -d`, {
        encoding: "utf8",
      });
      await writePath(`${containersPath}/.running`, {
        timestamp: new Date().toISOString(),
      });
    }

    if (action === "stop") {
      execSync(`cd ${containersPath} && docker-compose down`, {
        encoding: "utf8",
      });
      const fs = await import("fs/promises");
      try {
        await fs.unlink(`${containersPath}/.running`);
      } catch {}
    }
